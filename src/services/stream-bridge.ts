...
/**
 * Ponte de stream browser<->Node usada para fazer requisições de chat ao
 * chat.qwen.ai DENTRO da página logada (fetch same-origin com o fingerprint
...
export async function browserFetch(
  ...
  try {
    ...
    if (err.message?.includes('Execution context was destroyed') || err.message?.includes('Target closed') || err.message?.includes('Page is closed')) {
      throw new Error(`browserFetch context lost: ${err.message}`);
    }
    throw err;
  }
}
...
export async function browserStreamFetch(
  ...
  const metaTimeoutMs = options.timeoutMs || 120000;
